...
fpkm0 <- rpkm(dgList)
fpkm1 <- removeBatchEffect(fpkm0, batch = batch)

fpkm <- subset(fpkm1, rowSums(sign(fpkm1) < 0) == 0)
...
#### BATCH CODE ####
design.mat <- model.matrix(~ 0 + dgList$samples$group + batch)
colnames(design.mat) <- c(levels(dgList$samples$group), "B2")
#### END ####
...
resdata <- merge(as.data.frame(edgeR_results), as.data.frame(fpkm1), by="row.names")
...
svg("edgeR-heatmap-samples.svg", w=8, h=8)
...
logCPM_bc <- removeBatchEffect(cpm(dgList, log = TRUE), batch=batch)

write.csv(fpkm1, file = paste0("edgeR_FPKM_", comp, ".csv"))
write.csv(logCPM_bc, file = paste0("edgeR_log2CPM_", comp, ".csv"))
write.csv(cpm(dgList, log = FALSE), file = paste0("edgeR_CPM_", comp, ".csv"))